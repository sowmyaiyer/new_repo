...
i <- 0
while ((diff1 > 0.0001 | diff2  > 0.0001) & (i < 1000))
{
	...
	for (i in 1:nrow(multimap_readCounts))
        {
                ...
                for (gene in genes_vector)
                {
                        ...
                        genes_t_plus_1_df[gene,"multireadassignment"] <- genes_t_plus_1_df[gene,"multireadassignment"] + (multiplicity * (genes_t_df[gene,"snatch_preference"]/sum_snatch_preference)*(genes_t_df[gene,"snatch_contribution_normalized"]/sum_snatch_contribution))
                }
        }
	...
	diff2 <- sum(abs(genes_t_plus_1_df$snatch_contribution_normalized - genes_t_df$snatch_contribution_normalized))
	i <- (i + 1)
	cat(diff1,diff2,"\n")
