...
do
	...
	}' /home/si14w/gnearline/flu/txt/${time}.CTSS.raw.bedGraph | sort -k1,1 -k2,2n > /home/si14w/gnearline/flu/txt/${time}_raw.bed
	#top5_thresh=`awk '{ print $5}' /home/si14w/gnearline/flu/txt/${time}_raw.bed | Rscript $HOME/gnearline/hdc/scripts/getQuantile.R`
	awk '{ if ($5 > 5) print }' /home/si14w/gnearline/flu/txt/${time}_raw.bed >  /home/si14w/gnearline/flu/txt/${time}_raw.tags_above_5.bed
#	bedtools merge -i  /home/si14w/gnearline/flu/txt/${time}_raw.tags_above_95thperc.bed -s -d 20 -c 4,5,5 -o collapse,collapse,sum | awk -f pickTss.awk | sort -k1,1 -k2,2n > /home/si14w/gnearline/flu/txt/${time}_raw.TSS.sorted.bed
# Use below line if we need to account for closeness to annotated TSS. Right now, only picking TSS with most tag counts.
#	awk -vthresh=$top5_thresh '{ if ($5 > thresh) print }' /home/si14w/gnearline/flu/txt/${time}_raw.bed | bedtools closest -a stdin -b ~/gnearline/common_scripts/hg19_refseq_rsem_corrected_tss.sorted.bed -d | bedtools merge -i stdin -s -d 100 -c 4,5,13,10 -o collapse,collapse,collapse,distinct 
done
