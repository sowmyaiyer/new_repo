...
write.table(genes_t_df, file=paste("/home/si14w/gnearline/flu/txt/genes_t_df_initial_snatch_contribution",time,".txt",sep=""),quote=FALSE, sep="\t")
...
while (diff1 > 0.0001 | diff2  > 0.0001)
{
	...
	for (i in 1:nrow(multimap_readCounts))
        {
                ...
                sum_snatch_contribution <- sum(genes_t_df[genes_vector,"snatch_contribution_unnormalized"])
                for (gene in genes_vector)
                {
                        ...
                        genes_t_plus_1_df[gene,"multireadassignment"] <- genes_t_plus_1_df[gene,"multireadassignment"] + (multiplicity * (genes_t_df[gene,"snatch_preference"]/sum_snatch_preference)*(genes_t_df[gene,"snatch_contribution_unnormalized"]/sum_snatch_contribution))
                }
        }
	...
	diff1 <- sum(abs(genes_t_plus_1_df$snatch_preference - genes_t_df$snatch_preference))
	diff2 <- sum(abs(genes_t_plus_1_df$snatch_contribution_unnormalized - genes_t_df$snatch_contribution_unnormalized))
	cat(diff1,diff2,"\n")
	genes_t_df <- genes_t_plus_1_df
}
write.table(genes_t_df, file=paste("/home/si14w/gnearline/flu/txt/genes_t_df_final_snatch_contribution",time,".txt",sep=""),quote=FALSE, sep="\t")
