if [[ -f ../txt/H1hesc_TF_intersection_stats.txt ]]; then
	rm ../txt/H1hesc_TF_intersection_stats.txt
fi
...
echo tfname peaks_intersecting_atac_enhancers peaks_intersecting_atac_nonenhancers peaks_intersecting_atac_both_enh_and_nonenh peaks_not_intersecting_atac total_peaks > ../txt/H1hesc_TF_intersection_stats.txt
...
#	peaks_intersecting_atac_all=`zcat $tfgz | bedtools intersect -a stdin -b /home/si14w/gnearline/ATAC_analysis/data/H1_ATAC.distal.sorted.bed -u | wc -l`
	peaks_not_intersecting_atac=`zcat $tfgz | bedtools intersect -a stdin -b /home/si14w/gnearline/ATAC_analysis/data/H1_ATAC.distal.sorted.bed -v | wc -l`
	echo $tfname $peaks_intersecting_atac_enhancers $peaks_intersecting_atac_nonenhancers $peaks_intersecting_atac_both_enh_and_nonenh $peaks_not_intersecting_atac $total_peaks >> ../txt/H1hesc_TF_intersection_stats.txt
done
sed 's/ /\t/g' ../txt/H1hesc_TF_intersection_stats.txt > ../txt/tmp
mv ../txt/tmp ../txt/H1hesc_TF_intersection_stats.txt
...
echo  TF enhancers_bound_to_tf total_enhancers totalTFPeaks > ../txt/enhancer_intersecting_tf.txt
echo  TF nonenhancers_bound_to_tf total_nonenhancers totalTFPeaks > ../txt/nonenhancer_intersecting_tf.txt
for tfgz in ../data/ENCS*gz
do
	tfname=`basename $tfgz | cut -d"_" -f4 | cut -d"." -f1`
	...
	echo $tfname $enhancers_intersecting_tf $total_enhancers $totalPeaks >> ../txt/enhancer_intersecting_tf.txt
	echo $tfname $nonenhancers_intersecting_tf $total_nonenhancers $totalPeaks >> ../txt/nonenhancer_intersecting_tf.txt
done

bedtools intersect -a ../data/H1_ATAC.distal.sorted.bed -b ../data/H1hesc.AllTFPeaks.bed -wa -wb | head | awk 'BEGIN{OFS="\t"}{ print $16,$5}' > ../txt/TFs_and_ATAC_scores.txt

bedtools intersect -a ../data/H1hesc.AllTFPeaks.bed -b ../data/H1_ATAC_intersect_H3K27ac.bed | awk 'BEGIN{OFS="\t"}{ print $11,$7,"enhancer_ATAC"}' > ../txt/H1_TFPeaks_in_enhancer_atac_peaks.txt
bedtools intersect -a ../data/H1hesc.AllTFPeaks.bed -b ../data/H1_ATAC_non_intersect_H3K27ac.bed | awk 'BEGIN{OFS="\t"}{ print $11,$7,"non_enhancer_ATAC" }' > ../txt/H1_TFPeaks_in_nonenhancer_atac_peaks.txt
bedtools intersect -a ../data/H1hesc.AllTFPeaks.bed -b ../data/H1_ATAC_intersect_H3K27ac.bed -v | awk 'BEGIN{OFS="\t"}{ print $11,$7,"non_ATAC"}' > ../txt/H1_TFPeaks_not_in_atac_peaks.txt
Rscript tfPeakAnalysis.R
